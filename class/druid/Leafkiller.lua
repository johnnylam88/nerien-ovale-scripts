...
AddFunction RakeVsMangleTargetNotDieing asValue=1 { FutureRakeTickDamage() * {target.TicksRemain(RAKE)+1} - LastRakeTickDamage() * target.TicksRemain(RAKE) >= MangleCatDamage() }
AddFunction RakeVsMangleTargetIsDieing asValue=1 { FutureRakeTickDamage() * {target.TimeToDie()/3+1} - LastRakeTickDamage() * target.TimeToDie()/3 >= MangleCatDamage() }
AddFunction FillerActions
{
    ...
    if target.TimeToDie() - target.DebuffRemains(RAKE) >=3 and RakeVsMangleTargetNotDieing()
    {
        Spell(RAKE)
    }
    ...
    if target.TimeToDie() - target.DebuffRemains(RAKE) <3 and RakeVsMangleTargetIsDieing()
    {
        Spell(RAKE)
    }
    #ravage
