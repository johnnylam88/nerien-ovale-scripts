...
Define(chain_reaction_buff 278310)
	...
	SpellAddBuff(ice_lance chain_reaction_buff add=1 enabled=(Talent(chain_reaction_buff) and BuffPresent(fingers_of_frost_buff)))
Define(cold_snap 235219)
	SpellInfo(cold_snap cd=270)
Define(comet_storm 153595)
	SpellInfo(comet_storm cd=30)
	SpellRequire(comet_storm unusable set=1 enabled=(not Talent(comet_storm_talent)))
...
Define(ebonbolt 257537)
	...
	SpellRequire(ebonbolt unusable set=1 enabled=(not Talent(ebonbolt_talent)))
...
Define(freezing_rain_buff 270232)
	...
	SpellAddBuff(frozen_orb freezing_rain_buff add=1 enabled=(Talent(freezing_rain_talent)))
...
Define(glacial_spike 199786)
	...
	SpellRequire(glacial_spike unusable set=1 enabled=(not (Talent(glacial_spike_talent) and BuffStacks(icicles_buff) == 5)))
...
Define(ice_floes 108839)
	...
	SpellRequire(ice_floes unusable set=1 enabled=(not Talent(ice_floes_talent)))
Define(ice_lance 30455)
	SpellInfo(ice_lance travel_time=0.5)
Define(ice_nova 157997)
	SpellInfo(ice_nova cd=25)
	SpellRequire(ice_nova unusable set=1 enabled=(not Talent(ice_nova_talent)))
...
Define(icicles_buff 205473)
	...
	SpellAddBuff(ice_lance icicles_buff set=0 enabled=(SpellKnown(icicles) and not Talent(glacial_spike_talent)))
Define(icy_veins 12472)
	...
	SpellRequire(icy_veins duration add=10 enabled=(Talent(thermal_void_talent)))
...
Define(ray_of_frost 205021)
	...
	SpellRequire(ray_of_frost unusable set=1 enabled=(not Talent(ray_of_frost_talent)))
...
Define(rune_of_power_buff 116014)
	...
	SpellAddBuff(icy_veins rune_of_power_buff add=1 enabled=(Talent(rune_of_power_talent)))
...
Define(summon_water_elemental 31687)
	...
	SpellRequire(summon_water_elemental unusable set=1 enabled=(Talent(lonely_winter_talent)))
