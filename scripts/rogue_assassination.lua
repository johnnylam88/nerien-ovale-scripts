...
Define(blindside_buff 121153)
	...
	SpellAddBuff(ambush blindside_buff set=0 enabled=(Talent(blindside_talent)))
...
Define(hidden_blades_buff 270070)
	SpellAddBuff(fan_of_knives hidden_blades_buff set=0 enabled=Talent(hidden_blades_talent))
...
Define(stealth_subterfuge_buff 115191)
	SpellAddBuff(stealth stealth_subterfuge_buff add=1 enabled=(Talent(subterfuge_talent)))
	SpellDamageBuff(garrote stealth_subterfuge_buff set=1.8 enabled=(Talent(subterfuge_talent)))
Define(subterfuge_buff 115192)
	SpellInfo(subterfuge_buff duration=3)
	SpellDamageBuff(garrote subterfuge_buff set=1.8 enabled=(Talent(subterfuge_talent)))
...
Define(vanish_buff 11327)
	...
	SpellDamageBuff(garrote vanish_buff set=1.8 enabled=(Talent(subterfuge_talent)))
...
AddFunction AssassinationPrecombatMainActions
{
	...
	if (Talent(subterfuge_talent) and Stealthed()) Spell(garrote)
...
AddFunction AssassinationPrecombatAoEActions
{
	if (BuffRemaining(lethal_poison_buff) < 1200) Spell(deadly_poison)
	Spell(slice_and_dice)
	if (Talent(subterfuge_talent) and Stealthed()) Spell(garrote text=cycle)
	Spell(serrated_bone_spike)
	Spell(fan_of_knives)
}
...
AddFunction AssassinationMainActions
{
	...
	if SpellCooldown(vanish) > 0 or Talent(subterfuge_talent)
	{
		if (ComboPointsDeficit() <= 1) AssassinationFinisherActions()
		if (ComboPointsDeficit() > 1) AssassinationBuilderActions()
	}
	if not SpellCooldown(vanish) > 0 and (Talent(nightstalker_talent) or Talent(master_assassin_talent))
	{
